struct VertexOutput {
    ...
    @location(0) @interpolate(flat) vertex_index: i32,
...
fn vertex(
    ...
    output.barycentrics[i32(vertex_index) % 3] = 1.0;
...
fn fragment(
    ...
    //var offset = 0.0001;
    // Conversative rasterisation can push the barycentric coordinates out of range.
    //var barycentrics = clamp(barycentrics, vec3(offset), vec3(1.0-offset));
    //var barycentric_sum = barycentrics.x + barycentrics.y + barycentrics.z;
    //barycentrics /= barycentric_sum;
    ...
    var frag_position = interpolate(pos_a, pos_b, pos_c, barycentrics);
    var frag_normal = interpolate(normal_a, normal_b, normal_c, barycentrics);

    var offset_a = compute_vector_and_project_onto_tangent_plane(frag_position, pos_a, normal_a);
    var offset_b = compute_vector_and_project_onto_tangent_plane(frag_position, pos_b, normal_b);
    var offset_c = compute_vector_and_project_onto_tangent_plane(frag_position, pos_c, normal_c);

    var interpolated_offset = interpolate(offset_a, offset_b, offset_c, barycentrics);

    var output : FragmentOutput;
    var norm: vec3<f32> = normalize(frag_normal);
    output.positionOutput = vec4<f32>(frag_position + interpolated_offset, 1.0);
